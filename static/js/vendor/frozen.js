 ...
 * Time:11:03
 * Version:1.0
 * For: evaluation star mobile end
 */
(function($, window){
    var _startTpl = "<span class='star ui-icon-qqlevel ui-icon-qqlevel-none'></span>";
    ...
    function addStar(ele){
        ele.removeClass("ui-icon-qqlevel-none");
        ele.addClass("ui-icon-qqlevel-star");
    }

    function cancelStar(ele){
        ele.removeClass("ui-icon-qqlevel-star");
        ele.addClass("ui-icon-qqlevel-none");
    }

    var Star = function(ele, option){
    ...
    Star.prototype = {
        _init: function(){
            var startList = [];
            for(var i=0; i < this.options.startCount; i++ ){
                 startList.push(_startTpl);
            }
            var startHtml = startList.join("");
            $(startHtml).appendTo(this.element);
        },

        _bindEvent:function(){
            this.element.find(".ui-icon-qqlevel").click(function(){

                var first = $(this);
                var last = $(this);
                addStar(last);
                while(last.prev().hasClass("star")){
                    addStar(last.prev());
                    last = last.prev();
                }

                while(first.next().hasClass("star")){
                    cancelStar(first.next());
                    first = first.next();
                }
        ...
        getStarNum:function(){
            return this.element.find(".ui-icon-qqlevel-star").length;
        }
    ...
    function Plugin(option) {
		...
		return $.adaptObject(this, defaults, option,"",Star,"star");
	}
	$.fn.star=$.star= Plugin;
